...
@step_log("validation_and_get_data")
def validation_and_get_data(state: ImportGraphState) -> tuple[list[dict[str, Any]], str]:
    ...
    if not file_title:
        if md_path and Path(md_path).exists():
            file_title = Path(md_path).name
        else:
            file_title = "default_title"
            logger.warning(f"file_title没有值，给予默认值default_title")
    ...
    if not chunks:
        ...
        if not chunks:
            logger.error(f"chunks 为空，读取本地备份文件后依旧为空，业务无法继续！")
            raise ValueError(f"chunks 为空，读取本地备份文件后依旧为空，业务无法继续！")

    return chunks, file_title

@step_log("recognize_item_name_by_chunks")
def recognize_item_name_by_chunks(chunks:list[dict[str, Any]], file_title:str) -> str:
    chat_client = llm_provider.chat()
    system_prompt: str = load_prompt("product_recognition_system")
    user_context: str = ""
    ...
    """
        context = 
            第1 2 3 chunk: 标题：title，内容：content \n
    """
    ...
    user_prompt = load_prompt("item_name_recognition", file_title=file_title, context=user_context)
    message = [
        SystemMessage(
    ...
    chains = chat_client | StrOutputParser()
...
def chunk_update_item_name(chunks: list[dict[str, Any]], item_name: str):
...
def prepared_milvus_item_name_collection():
    ...
    # 1.判断是否有集合
    has_item_name_collection = milvus_client.has_collection(item_name_collection_name)
...
def recognize_and_index_item_name(state: ImportGraphState) -> ImportGraphState:
    ...
    # 1.获取并校验参数
    chunks, file_title = validation_and_get_data(state)
