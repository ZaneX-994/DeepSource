...
from app.infra.persistence.redis_doc_registry import registry
...
# ==================redis文件去重==================
def file_hash(filepath: Path, algorithm: str = "sha256", chunk_size: int = 8192) -> str:
    """计算文件的二进制哈希值"""
...
def parse_pdf_to_markdown(state: ImportGraphState) -> ImportGraphState:
    ...
    # 存在校验
    hash_value = file_hash(pdf_path_obj)
    state["file_hash"] = str(hash_value)
    res = registry.register(pdf_path_obj.name, hash_value)
...
def upload_pdf_and_poll(pdf_path_obj:Path) -> str:
    ...
    try:
        ...
        # 1. 向minerU服务器发送请求申请上传地址 (batch_id / url)
        header = {
        ...
        # 🔴 失败补偿：精确反注册，而非暴力 delete
        logger.error(f"上传流程失败, 正在回滚Redis注册记录(file={pdf_path_obj.name}, hash={state['file_hash']}), 原始错误: {e}")
        try:
            registry.unregister(pdf_path_obj.name, state['file_hash'])
            logger.info(f"Redis注册记录回滚成功: file={pdf_path_obj.name}, hash={state['file_hash']}")
        except Exception as rollback_err:
            logger.error(f"🔥 Redis回滚失败! file={pdf_path_obj.name}, hash={state['file_hash']}, "
                        f"回滚错误: {rollback_err}, 原始错误: {e}")
            raise  # 重新抛出原始异常
