from pymilvus import has_collection, DataType

from app.infra.vector_store.milvus_gateway import MilvusGateway
...
def require_embeddings_content(state: ImportGraphState) -> list[dict]:
    # 从 state 中获取核心数据
...
def prepare_chunks_collection() -> None:
    ...
    # 为稠密向量创建索引：使用 HNSW，metric_type 为 COSINE
...
def index_chunks(state: ImportGraphState) -> ImportGraphState:
    ...
    # 先校验切片存在，避免把空数据写入向量库
    embedding_content = require_embeddings_content(state)
    ...
    # 批量插入新数据
    insert_embeddings_content(embedding_content)
